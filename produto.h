...
Produto();
 ...
 Produto(int id_produto, const char nome[], float preco, int estoque);
  
  void setId_produto(int id_produto);
  int getId_produto();
...
void inicializartabela(celula T[MAX]) {//antes de adicionar as chaves, fiz com que o estado da tabela sejam todos VAZIOS
    for (int i = 0; i < MAX; i++) {
...
int busca_aberto(int m, celula T[MAX], int x) {
    ...
    do {
        ...
        if (T[j].chave == x && T[j].estado == 1) { // 1->OCUPADA
            return j;
        } else {
            i++;
        }
...
int insere_aberto(int m, celula T[MAX], int x) {//m-> MAX (tamanho da tabela), x->chave (id do produto)
    int i, j;
    i = 0;
    do {
        j = h(x, i);
